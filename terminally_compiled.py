...
class myclass():
    @staticmethod
    def find_all(instance,time):
        ROOT.maintab.output("hello world")
        return str(instance), int(time)
    @staticmethod
    def testertest():
        ROOT.maintab.output("this works\nso\ndoes")

    @staticmethod
    def finaltest():
        ROOT.maintab.output(247)
    @staticmethod
    def rick_roll():
...
class name_ager():
    ...
    @staticmethod
    def verify(name, age):
        ROOT.maintab.output(name,type(name),age,type(age))
        ROOT.maintab.output(name_ager.correct_name_age)
        if (name,age) == name_ager.correct_name_age:
            ROOT.maintab.output("Age supplied was correct")
        else:
            ROOT.maintab.output("Age supplied was wrong")



CLASSES = {'myclass':myclass,'name_ager':name_ager}
